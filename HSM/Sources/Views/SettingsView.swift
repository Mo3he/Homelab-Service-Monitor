...
struct SettingsView: View {
    ...
    private var feedbackURL: URL {
        ...
        let subject = "HSM Feedback – v\(version)"
        ...
        NavigationStack {
            Form {
                ...
                Section {
                    ...
                    Text("How often HSM refreshes your services while in the background. iOS may delay or skip refreshes to preserve battery.")
                }
                ...
                Section("Data") {
                    if let data = vm.exportJSON() {
                        let jsonFile = JSONFile(data: data, filename: "hsm-services.json")
                        ShareLink(
                            item: jsonFile,
                            preview: SharePreview("HSM Services", image: Image(systemName: "square.and.arrow.up"))
                        ) {
                    ...
                    let reportFile = HTMLFile(data: reportHTML, filename: "hsm-report.html")
                    ShareLink(
                        item: reportFile,
                        preview: SharePreview("HSM Status Report", image: Image(systemName: "chart.bar.doc.horizontal"))
                    ) {
