...
struct RefreshAllServicesIntent: AppIntent {
    ...
    static var description = IntentDescription("Refreshes all HSM services immediately and returns an overview.")
    static var openAppWhenRun = false

    func perform() async throws -> some IntentResult & ProvidesDialog {
        let services = await MainActor.run { ServiceStore.shared.services }
        guard !services.isEmpty else {
            return .result(dialog: "No services configured in HSM.")
        }
...
struct HSMShortcuts: AppShortcutsProvider {
